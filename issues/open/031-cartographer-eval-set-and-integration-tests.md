...
**AC-C6 deferred.** Negative-fixture coverage (grid-cross-validation failure, engineered overlap, zero-detections) is dropped from this issue. The pipeline already raises typed exceptions and writes diagnostic PNGs for these cases by construction (see issues 028/029); confirming that contract via integration tests is a follow-up if the failure modes need explicit regression coverage.
...
- [x] 5 eval screenshots committed under `app/data/cartographer_eval/` with hand-labeled ground-truth `BaseLayout` JSONs (paired `<name>.jpg` + `<name>.json`), each layout including every wall tile.
- [x] `requires_roboflow_api_key` pytest marker registered and skips cleanly when unset.
- [x] Integration tests `tests/integration/cartographer/test_eval_set.py` exist, parametrize over the 5 fixtures, and encode AC-C1/C3/C4/C5/C7 as assertions. Skip cleanly when `ROBOFLOW_API_KEY` is unset.

**Deferred to issue #36 (grid robustness on real screenshots):** the empirical AC bars below cannot be measured until the grid stage succeeds on the eval set. As of this issue's closure, `pipeline.run()` raises `GridCrossValidationError` on all 5 fixtures (axis-pitch disagreement 5.7 % – 159 % vs the §4.3 2 % threshold). The integration tests stand ready as the regression bar that closes #36.

- [ ] AC-C1 met (every ingestion loads in sim) — gated on #36.
- [ ] AC-C3 met (≥90% F1 class, ≥95% IoU bbox) — gated on #36.
- [ ] AC-C4 met (grid within ±0.5 tile on all 5) — gated on #36.
- [ ] AC-C5 met (wall ≥95% precision, ≥90% recall) — gated on #36.
- [ ] AC-C7 met (provenance round-trip on every success) — gated on #36.

## Followed up by

- `issues/open/036-cartographer-grid-robustness-on-real-screenshots.md`
