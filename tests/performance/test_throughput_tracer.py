"""Performance benchmark: tracer scenario throughput.

Runs the tracer_smoke scenario (tracer.json + single_barb.json) repeatedly on a
single core and asserts ≥ 100 episodes/sec per §12.1.
"""
...
THRESHOLD_EP_SEC = 100.0
MIN_DURATION_S = 1.0

_BASE_PATH = DEFAULT_DATA_DIR / "sample_bases" / "tracer.json"
_PLAN_PATH = DEFAULT_DATA_DIR / "sample_plans" / "single_barb.json"
...
@pytest.mark.slow
def test_throughput_tracer() -> None:
    sim = _build_sim()
    ...
    assert ep_sec >= THRESHOLD_EP_SEC, (
        f"[tracer_smoke] throughput {ep_sec:.1f} ep/sec"
